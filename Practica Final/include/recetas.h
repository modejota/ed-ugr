...
/**
  * @brief Sobrecarga del operador [] para poder acceder a una receta directamente dado su codigo. No constante
  * @param cod Codigo de la receta a la que queremos acceder
  * @return Devuelve la receta asociada al codigo pasado como parametro. Si no se encuentra devuelve la por defecto
  */
        receta operator[](const string &cod);

/**
  * @brief Sobrecarga del operador [] para poder acceder a una receta directamente dado su codigo. Constante
  * @param cod Codigo de la receta a la que queremos acceder
  * @return Devuelve la receta asociada al codigo pasado como parametro. Si no se encuentra devuelve la por defecto
  */
...
/**
  ...
  */
	...
	void setNutricion(Ingredientes &I);

/**
  * @brief Inserta un elemento en el map de Recetas
  * @param elemento Receta que vamos a insertar en el map
  */
        void insertar(pair<string,receta> &elemento);

        
/**
  * @brief Saca por el flujo solamente el codigo, el nombre y el plato de todas las recetas
  * @param os Flujo de salida
  * @return Devuelve el propio flujo de salida
  */
        ostream & informacionResumidaRecetas(ostream & os);
/**
  * @brief Comprueba si hay alguna receta con el codigo pasado como parametro
  * @param codigo un string, que es el codigo a comprobar
  * @return Devuelve un true si existe la receta, false si no existe 
  */
...
/**
  * @brief Metodo para cargar las recetas asi como las instrucciones para realizarlas
  * @param nombre Path de recetas.txt
  * @param path_ins Path del directorio de instrucciones
  * @return Devuelve si la operacion tiene exito
  */
  	bool CargarRecetas(const string &nombre, const string &path_ins);


/**
  * @section Iterator_recetas Iterador no constante de la clase Recetas.
  * Nos permite desplazarnos comodamente a traves del map con el conjunto de recetas
  */

/**
  * @brief Iterador no constante de recetas.
  */ 
        class iterator{
            private:
...
/**
  * @section Const_iterator_recetas Iterador constante de la clase Recetas.
  * Nos permite desplazarnos comodamente a traves del map con el conjunto de recetas
  */

/**
  * @brief Iterador constante de recetas.
  */ 
...
/**
  ...
  */
	const_iterator begin() const { const_iterator i; i.it = datos.cbegin(); return i; }

/**
  * @brief Crea un const_iterator y lo coloca en la primera posicion fuera del map de recetas
  * @return Devuelve el const_iterator creado
  */
	const_iterator end() const { const_iterator i; i.it = datos.cend(); return i; }
